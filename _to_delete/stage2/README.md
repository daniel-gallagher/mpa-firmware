...
3. **Quit the IDE completely and delete its board-menu cache.** Arduino IDE 2.x
   caches the parsed board menu and a plain restart will *not* pick up a
   changed `boards.txt` — the new USB Type simply won't appear. On macOS:

   ```
   rm -rf ~/Library/Application\ Support/arduino-ide
   ```

   (`~/.config/arduino-ide` on Linux, `%APPDATA%\arduino-ide` on Windows.) That
   loses window layout and recent-sketch history — not sketches, preferences,
   installed boards or libraries. The installer prints the right path for your
   machine.

   Note the entries go into `boards.txt`, not `boards.local.txt`. The latter is
   the documented way to extend a platform without touching vendor files and it
   works in IDE 1.8, but IDE 2.x does not read it reliably for this platform.

4. Start the IDE. **Tools → USB Type** now has:
...
5. Edit `firmware/teensympa/config.h` (drum module profile, D-pad type), open
