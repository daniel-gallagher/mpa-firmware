...
# Menu entries go into boards.txt, NOT boards.local.txt.
#
# boards.local.txt is the documented way to extend a platform without touching
# vendor files, and Arduino IDE 1.8 honours it. Arduino IDE 2.x does not read
# it reliably for this platform - the entries simply never appear in the Tools
# menu. Editing boards.txt directly is the only approach that works in both.
#
# Property set matches a stock Teensy entry exactly:
#   build.usbtype        -> the -D passed to the compiler
#   upload_port.usbtype  -> how the IDE matches a detected port to this board;
#                           omit it and the entry misbehaves
#   fake_serial          -> tells the IDE not to expect a real serial port.
#                           Present on MPA (no serial at all), absent on the
#                           debug variant (SEREMU provides an emulated one).
BOARDS_MARKER_BEGIN = "# >>> mpa-firmware: MPA USB types (added by install_mpa_core.py) >>>"
BOARDS_MARKER_END   = "# <<< mpa-firmware: MPA USB types <<<"

BOARDS_BLOCK = "\n" + BOARDS_MARKER_BEGIN + """
teensy41.menu.usb.mpa=MPA (Rock Band drum adapter)
teensy41.menu.usb.mpa.build.usbtype=USB_MPA
teensy41.menu.usb.mpa.upload_port.usbtype=USB_MPA
...
teensy41.menu.usb.mpadbg.upload_port.usbtype=USB_MPA_SEREMU

teensy40.menu.usb.mpa=MPA (Rock Band drum adapter)
teensy40.menu.usb.mpa.build.usbtype=USB_MPA
teensy40.menu.usb.mpa.upload_port.usbtype=USB_MPA
...
teensy40.menu.usb.mpadbg.upload_port.usbtype=USB_MPA_SEREMU
""" + BOARDS_MARKER_END + "\n"

# Where Arduino IDE 2.x caches its parsed board/menu data. Editing boards.txt
# is NOT picked up by a plain restart - the IDE has to be quit and this
# directory removed. Deleting it loses window layout and recent-sketch history;
# it does not touch sketches, preferences, installed boards or libraries.
IDE2_CACHE_DIRS = [
    "~/Library/Application Support/arduino-ide",       # macOS
    "~/.config/arduino-ide",                           # Linux
    "%APPDATA%\\arduino-ide",                          # Windows
]


def _version_key(path):
    """Sort Teensyduino version directories numerically.

    Plain string sorting gets this wrong the moment a component reaches two
    digits - '1.9.0' sorts above '1.10.0'. Not a problem today (1.62.0), but
    it would silently patch the wrong install later.
    """
    import re
    m = re.search(r"/(\d+(?:\.\d+)*)/cores/", path)
    if not m:
        return (0,)
...
def find_core_dir():
    ...
    for pat in CANDIDATE_CORES:
        hits = [h for h in glob.glob(os.path.expanduser(pat))
                if os.path.isfile(os.path.join(h, "usb_desc.h"))]
        if hits:
            hits.sort(key=_version_key, reverse=True)
            if len(hits) > 1:
                print("Multiple Teensyduino versions found; using the newest:")
                for h in hits:
                    print(f"  {'->' if h == hits[0] else '  '} {h}")
            return hits[0]
    return None
...
def install_boards(boards_dir, uninstall=False, check=False):
    path = os.path.join(boards_dir, "boards.txt")
    if not os.path.isfile(path):
        return [], [f"no boards.txt in {boards_dir}"]

    with open(path, "r", encoding="utf-8", errors="surrogateescape") as f:
        text = f.read()
    present = BOARDS_MARKER_BEGIN in text

    if uninstall:
        if not present:
            return [], []
        start = text.index(BOARDS_MARKER_BEGIN)
        end = text.index(BOARDS_MARKER_END) + len(BOARDS_MARKER_END) + 1
        # also swallow the blank line we inserted before the marker
        ...
        if not check:
            with open(path, "w", encoding="utf-8", errors="surrogateescape") as f:
                f.write(new)
        return [f"  reverted: {path} (menu entries removed)"], []
    ...
    if not check:
        ...
        if not text.endswith("\n"):
            text += "\n"
        with open(path, "w", encoding="utf-8", errors="surrogateescape") as f:
            f.write(text + BOARDS_BLOCK)

    # An earlier version of this script wrote the entries to boards.local.txt.
    # Leave a note rather than duplicate menu items if that file is still there.
    stale = os.path.join(boards_dir, "boards.local.txt")
    extra = []
    if os.path.exists(stale):
        try:
            if "menu.usb.mpa" in open(stale).read():
                extra.append(f"  NOTE: {stale} also contains MPA entries from an "
                             f"older run - empty or delete it to avoid duplicates")
        except OSError:
            pass
    return [f"  patched: {path} (USB Type menu entries)"] + extra, []
...
def main():
    ...
    if not args.check:
        print()
        print("!! Arduino IDE 2.x will NOT pick this up from a plain restart.")
        print("   It caches the parsed board menu. Quit the IDE completely, then")
        print("   remove whichever of these exists:")
        for d in IDE2_CACHE_DIRS:
            expanded = os.path.expanduser(d)
            mark = "  <- this one" if os.path.isdir(expanded) else ""
            print(f"     {expanded}{mark}")
        print("   (that loses window layout and recent-sketch history only -")
        print("    not sketches, preferences, boards or libraries)")
        print("   Then start the IDE again.")
    if not args.uninstall and not args.check:
        print()
        print('Tools > USB Type should now list "MPA (Rock Band drum adapter)".')
    return 0
